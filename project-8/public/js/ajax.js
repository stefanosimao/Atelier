...
function buttonClick(event) {
    ...
    else if (buttonName == 'addall') {
        let addToPlaylist = {id: id};
        socket.emit("addToPlaylist", addToPlaylist);
    }
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
API = function () {
    ...
    function getEdit(id) {
        ...
        document.getElementById('cancel').addEventListener("click", (event)=>{
            socket.emit("stop_edit", msg);
            getSongs();
        });
        
        changeLocationBar('#edit', id);
        socket.emit("start_edit", msg);
...
function init_player_with_playlist(dom, key, a) {
                ...
                <section class="player" id='playerF'>
                    <main class="player" id="player${key}">
                        ...
                        <aside class="player volume" > 
                                ...
                                <button type="button" id="noadd${key}" class="linkon"></button>
    ...
    function generateStyle(key) {
        ...
        width: 95%;}
        #player${key} button#noadd${key}.linkon {background-image: url("images/link.svg")}
        #player${key} button#noadd${key}.linkoff {background-image: url("images/link_off.svg")}
        ...
        #player${key} #noadd${key} {grid-column: 1; grid-row: 5; margin-left: 1em}
    ...
    function noadd_button(key) {
        if (no_add) {
            no_add = false;
            document.querySelector(`#noadd${key}`).classList.remove('linkoff');
            document.querySelector(`#noadd${key}`).classList.add('linkon')
        }
        else {
            no_add = true;
            document.querySelector(`#noadd${key}`).classList.remove('linkon');
            document.querySelector(`#noadd${key}`).classList.add('linkoff')
        }
    }
    ...
    document.querySelector(`#noadd${key}`).addEventListener('click', () => noadd_button(key));
...
function init_socket(){
    ...
    socket.on('song.created', (new_song) =>{
        song_data.push(new_song);
        let frag = window.location.hash;
        navigate(frag);
    ...
    socket.on('addNowToPlaylist', (addToPlaylist) => {
        if (!no_add) {
            API.addToPlaylist(addToPlaylist.id);
        }else{
    ...
    socket.on("start_play", () => {
        console.log('START');
        document.querySelector(`#player-audiopl`).play();
    });

    socket.on("pause_play", () => {
        console.log('PAUSE');
        document.querySelector(`#player-audiopl`).pause();
    });

    socket.on('song.modified', (mod_song) => {
        let index;
        let f_name;
        let f_title;
        for (let i = 0; i < song_data.length; i++) {
            if (song_data[i]._id == mod_song._id) {
                index = i;
                f_name = song_data[i].filename;
                f_title = song_data[i].title;
            }
        ...
        let frag = window.location.hash;
        navigate(frag);
        console.log(`Song ${mod_song.title} modified!`);
    });

    socket.on('song.deleted', (event) => {
        let id = event._id;
        let found = false;
        for (let i = 0; i < song_data.length; i++) {
            if (song_data[i]._id == id) {
    ...
    socket.on('editing', () => {
        console.log('EDIT');
        document.getElementById('cancel').value = 'SOMEONE ELSE IS EDITING THIS SONG!!'   
    });

    socket.on('stop_editing', () => {
        document.getElementById('cancel').value = 'X'
    }); 
